...
content.each { |ligne|
 ...
 # elle commence bien 
# puts ligne
  if (ligne =~ /^"Projet/) or (ligne =~ /^"Chantier/)  then 
     if !tl.empty?  then 
       @tm_content << tl
     end  
     tl = ligne
...
@content.shift
@content.shift
## lire le tableau une fois et gerer le multiligne
@content2 =[]
@content2 = formate_mlt(@content)
#puts @content2.inspect
#puts @content2 
@content = @content2
@content.each do |e| 
    e.gsub!(/(\d+),(\d+)/,'\1.\2')
